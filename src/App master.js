...
function App() {
  ...
  const xAxisTicks = useMemo(() => {
    ...
    return Array.from({ length: tickCount }, (_, i) => {
      const value = (totalProduction / (tickCount - 1)) * i;
      const x = (value / totalProduction) * chartWidth + MARGIN.left;
      return (
        <g key={i}>
  ...
  return (
    <div style={{ fontFamily: 'Arial, sans-serif' }}>
      ...
      <div>
        ...
        <p>First data point: {JSON.stringify(data[0])}</p>
        <p>Max cost: {formatNumber(maxCost, 2, '$')}/oz</p>
        <p>Total production: {formatNumber(totalProduction, 0)} koz</p>
        <p>Quartiles: Q1: {formatNumber(quartiles.q1, 2, '$')}, 
           Median: {formatNumber(quartiles.median, 2, '$')}, 
           Q3: {formatNumber(quartiles.q3, 2, '$')}</p>
      </div>
    </div>
  );
}
